...
export class ServicesService {
    ...
    public checkvalidateService = async (item: any, hotelId: any) => {
        ...
        if (typeof item.price === "undefined" || isNaN(item.price) || !item.price || item.price < 1) {
    ...
    // public upadate 
    public update = async (name: string, item: any, hotelId: string) => {
        try {

            try {

                const rs = await Repository.updateByName(name, item);
                if (rs) {
